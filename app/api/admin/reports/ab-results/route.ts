...
/**
 * GET /api/admin/reports/ab-results?window=30d
 * Proxy to backend GET /performance/ab
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/performance/ab?window=${window}`, {
    ...
    console.error('[AB Results API]', error)
