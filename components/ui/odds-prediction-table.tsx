...
/** Render star indicators for premium quality scoring */
function QualityStars({ stars, tier, signals }: { stars: number; tier: string; signals?: string[] }) {
  if (stars === 0) return null

  const starColors = {
    3: "text-amber-400",   // gold for premium
    2: "text-slate-300",   // silver for strong
    1: "text-slate-500",   // dim for standard
  }
  const color = starColors[stars as keyof typeof starColors] || "text-slate-500"
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent side="top" className="bg-slate-800 border-slate-700 text-xs max-w-[240px]">
          <div className="font-semibold text-white mb-1">{tierLabels[tier] || tier}</div>
      ...
      <td className="py-3 px-4 text-right">
        ...
        {match.predictions?.premium ? (
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <div className="flex items-center gap-2">
              {match.predictions.quality && match.predictions.quality.stars > 0 && (
                <QualityStars
                  stars={match.predictions.quality.stars}
                  tier={match.predictions.quality.tier}
                  signals={match.predictions.quality.signals}
                />
              )}
              {tier && (
                <Badge
