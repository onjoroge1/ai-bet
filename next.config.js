    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://www.google-analytics.com https://js.stripe.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://www.google-analytics.com https://vitals.vercel-insights.com https://api.stripe.com; frame-src https://js.stripe.com https://hooks.stripe.com; frame-ancestors 'none';",
          },
        ],
      },
      ...
      // ✅ FIX: Exclude user-specific routes from caching to prevent session crossover
      // User-specific routes should NEVER be cached (each user has unique data)
      {
        source: '/api/user/(.*)',
        headers: [
          {
      ...
      {
        source: '/api/predictions/history/(.*)',
        headers: [
          {
            key: 'Cache-Control',
            value: 'private, no-cache, no-store, must-revalidate',
      ...
      // Public API routes caching (non-user-specific)
      {
      ...
      {
        source: '/api/predictions/upcoming',
        headers: [
          {
            key: 'Cache-Control',
            value: 'public, max-age=300, s-maxage=300',
